...
export default function Dashboard({ user, onLogout }) {
  ...
  const mV=(v)=>hidden?"••••••":v; // mask value if hidden
  const KPI=({label,value,sub,color,icon})=>(<div style={{...cd,position:"relative",overflow:"hidden"}}><div style={{position:"absolute",top:0,left:0,right:0,height:3,background:color,borderRadius:"16px 16px 0 0"}}/><div style={{fontSize:11,color:"#94a3b8",fontWeight:600,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>{icon} {label}</div><div style={{fontSize:hidden?28:22,fontWeight:700,color:"#f1f5f9",...mn,letterSpacing:hidden?2:0,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{mV(value)}</div>{sub&&!hidden&&<div style={{fontSize:10,color:"#64748b",marginTop:3}}>{sub}</div>}</div>);
