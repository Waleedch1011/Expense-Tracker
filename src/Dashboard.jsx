...
export default function Dashboard({ user, onLogout }) {
  ...
  const KPI=({label,value,sub,color,icon})=>(<div style={{...cd,position:"relative",overflow:"hidden"}}><div style={{position:"absolute",top:0,left:0,right:0,height:3,background:color,borderRadius:"16px 16px 0 0"}}/><div style={{fontSize:11,color:"#94a3b8",fontWeight:600,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>{icon} {label}</div><div style={{fontSize:24,fontWeight:700,color:"#f1f5f9",...mn}}>{value}</div>{sub&&<div style={{fontSize:10,color:"#64748b",marginTop:3}}>{sub}</div>}</div>);
  ...
  return(<div style={{minHeight:"100vh",background:"linear-gradient(180deg,#0c0f1a 0%,#111827 50%,#0f172a 100%)",color:"#e2e8f0",fontFamily:"'Inter',-apple-system,sans-serif",padding:"20px 16px"}}>
    ...
    {showAdd&&<div style={{...cd,marginBottom:20,borderColor:"rgba(99,102,241,.3)"}}><ST icon="✏️">New Transaction</ST><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10}}>{[["Date","d","date"],["Type","t","select",settings.types],["Category","c","select",settings.categories],["Group","g","select",settings.groups],["Amount (PKR)","a","number"],["Description","desc","text"],["From Account","fa","select",["—",...settings.accounts]],["To Account","ta","select",["—",...settings.accounts]],["Party","p","select",["—",...settings.loan_people]],["Notes","n","text"]].map(([label,key,type,opts])=><div key={key}><label style={{fontSize:10,color:"#94a3b8",display:"block",marginBottom:3}}>{label}</label>{type==="select"?<select value={newTx[key]} onChange={e=>setNewTx({...newTx,[key]:e.target.value==="—"?"":e.target.value})} style={ip}>{opts.map(o=><option key={o} value={o==="—"?"":o}>{o}</option>)}</select>:<input type={type} placeholder={type==="number"?"0":"..."} value={newTx[key]} onChange={e=>setNewTx({...newTx,[key]:e.target.value})} style={ip}/>}</div>)}</div><div style={{marginTop:14,display:"flex",gap:10}}><button onClick={handleAdd} disabled={saving} style={{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",border:"none",color:"#fff",padding:"10px 24px",borderRadius:10,cursor:saving?"wait":"pointer",fontWeight:700,fontSize:13,opacity:saving?.6:1}}>{saving?"Saving...":"Save"}</button><button onClick={()=>setShowAdd(false)} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",color:"#94a3b8",padding:"10px 24px",borderRadius:10,cursor:"pointer",fontWeight:600,fontSize:13}}>Cancel</button></div></div>}
    ...
    <div style={{marginTop:28,padding:16,background:"rgba(255,255,255,.02)",borderRadius:16,border:"1px solid rgba(255,255,255,.04)",textAlign:"center"}}>
      ...
      <div style={{fontSize:28,fontWeight:800,...mn,background:"linear-gradient(135deg,#10b981,#06b6d4)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>₨ {ff(ACCOUNTS.reduce((s,a)=>s+a.balance,0))}</div>
      <div style={{fontSize:10,color:"#475569",marginTop:3}}>{ACCOUNTS.length} Accounts • Savings: ₨ {ff(SAVINGS_SUMMARY.reduce((s,x)=>s+x.totalBalance,0))} • {filtered.length} Transactions</div>
    </div>
  </div>);
}