...
export default function Dashboard({ user, onLogout }) {
  ...
  const MONTHLY_SUMMARY=useMemo(()=>["January","February","March","April","May","June","July","August","September","October","November","December"].map((month,mi)=>{
    const txs=norm.filter(tx=>new Date(tx.d).getMonth()===mi);
    let r={month,income:0,expense:0,loanGiven:0,loanTaken:0,loanRepaid:0,loanReceivedBack:0,officeExpense:0,personalExpense:0,transfer:0};
    txs.forEach(tx=>{if(tx.t==="Income")r.income+=tx.a;else if(tx.t==="Expense"){r.expense+=tx.a;if(tx.g==="Office")r.officeExpense+=tx.a;else r.personalExpense+=tx.a;}else if(tx.t==="Loan Given")r.loanGiven+=tx.a;else if(tx.t==="Loan Taken")r.loanTaken+=tx.a;else if(tx.t==="Loan Repaid")r.loanRepaid+=tx.a;else if(tx.t==="Loan Received Back")r.loanReceivedBack+=tx.a;else if(tx.t==="Transfer")r.transfer+=tx.a;});
    r.netMovement=r.income-r.expense; r.loanImpact=r.loanTaken-r.loanRepaid-r.loanGiven+r.loanReceivedBack; r.accountNet=r.netMovement+r.loanImpact; r.closingBalance=r.accountNet;
    return r;
  }),[norm]);
  ...
  const mV=(v)=>hidden?"••••••":v;
  const KPI=({label,value,sub,color,icon})=>(<div style={{...cd,position:"relative",overflow:"hidden",minHeight:90}}><div style={{position:"absolute",top:0,left:0,right:0,height:3,background:color,borderRadius:"16px 16px 0 0"}}/><div style={{fontSize:11,color:"#94a3b8",fontWeight:600,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>{icon} {label}</div><div style={{fontSize:hidden?26:18,fontWeight:700,color:"#f1f5f9",...mn,letterSpacing:hidden?3:0,wordBreak:"break-all",lineHeight:1.3}}>{mV(value)}</div>{sub&&!hidden&&<div style={{fontSize:10,color:"#64748b",marginTop:3}}>{sub}</div>}</div>);
  ...
  return(<div style={{minHeight:"100vh",background:"linear-gradient(180deg,#0c0f1a 0%,#111827 50%,#0f172a 100%)",color:"#e2e8f0",fontFamily:"'Inter',-apple-system,sans-serif",padding:"20px 16px"}}>
    ...
    {showAdd&&<div style={{...cd,marginBottom:20,borderColor:"rgba(99,102,241,.3)"}}><ST icon="✏️">New Transaction</ST><div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,marginBottom:10}}>{[["Date","d","date"],["Type","t","select",settings.types],["Category","c","select",settings.categories],["Group","g","select",settings.groups],["Amount (PKR)","a","number"]].map(([label,key,type,opts])=><div key={key}><label style={{fontSize:10,color:"#94a3b8",display:"block",marginBottom:3}}>{label}</label>{type==="select"?<select value={newTx[key]} onChange={e=>setNewTx({...newTx,[key]:e.target.value==="—"?"":e.target.value})} style={ip}>{opts.map(o=><option key={o} value={o==="—"?"":o}>{o}</option>)}</select>:<input type={type} placeholder={type==="number"?"0":"..."} value={newTx[key]} onChange={e=>setNewTx({...newTx,[key]:e.target.value})} style={ip}/>}</div>)}</div><div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>{[["Description","desc","text"],["From Account","fa","select",["—",...settings.accounts]],["To Account","ta","select",["—",...settings.accounts]],["Party","p","select",["—",...settings.loan_people]],["Notes","n","text"]].map(([label,key,type,opts])=><div key={key}><label style={{fontSize:10,color:"#94a3b8",display:"block",marginBottom:3}}>{label}</label>{type==="select"?<select value={newTx[key]} onChange={e=>setNewTx({...newTx,[key]:e.target.value==="—"?"":e.target.value})} style={ip}>{opts.map(o=><option key={o} value={o==="—"?"":o}>{o}</option>)}</select>:<input type={type} placeholder="..." value={newTx[key]} onChange={e=>setNewTx({...newTx,[key]:e.target.value})} style={ip}/>}</div>)}</div><div style={{marginTop:14,display:"flex",gap:10}}><button onClick={handleAdd} disabled={saving} style={{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",border:"none",color:"#fff",padding:"10px 24px",borderRadius:10,cursor:saving?"wait":"pointer",fontWeight:700,fontSize:13,opacity:saving?.6:1}}>{saving?"Saving...":"Save"}</button><button onClick={()=>setShowAdd(false)} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",color:"#94a3b8",padding:"10px 24px",borderRadius:10,cursor:"pointer",fontWeight:600,fontSize:13}}>Cancel</button></div></div>}
    ...
    {tab==="dashboard"&&<>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:12,marginBottom:20}}><KPI label="Total Income" value={"₨ "+fmt(stats.income)} color="#10b981" icon="💰"/><KPI label="Total Expense" value={"₨ "+fmt(stats.expense)} color="#ef4444" icon="💸"/><KPI label="Net Balance" value={"₨ "+fmt(stats.net)} sub={stats.net>=0?"Surplus":"Deficit"} color={stats.net>=0?"#10b981":"#ef4444"} icon="📊"/><KPI label="Total Savings" value={"₨ "+fmt(stats.savings)} color="#8b5cf6" icon="💎"/><KPI label="Account Balance" value={"₨ "+ff(ACCOUNTS.reduce((s,a)=>s+a.balance,0))} color="#06b6d4" icon="🏧"/></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:12,marginBottom:24}}><KPI label="Others Owe Me" value={"₨ "+ff(LOANS.filter(l=>l.net>0).reduce((s,l)=>s+l.net,0))} color="#f59e0b" icon="📤"/><KPI label="I Owe Others" value={"₨ "+fmt(Math.abs(LOANS.filter(l=>l.net<0).reduce((s,l)=>s+l.net,0)))} color="#ec4899" icon="📥"/><KPI label="Net Loan" value={"₨ "+ff(LOANS.reduce((s,l)=>s+l.net,0))} color="#f97316" icon="🔄"/><KPI label="Loan Impact" value={"₨ "+ff(stats.loanImpact)} color="#14b8a6" icon="⚡"/></div>
    ...
    {tab==="monthly"&&<div style={cd}><ST icon="📅">Monthly Summary (Jan–Dec)</ST><TW><thead><tr>{["Month","Income","Expense","Loan Given","Loan Taken","Loan Repaid","Loan Back","Net","Loan Impact","Acct Net","Office","Personal"].map(h=><th key={h} style={{...thS,fontSize:9}}>{h}</th>)}</tr></thead><tbody>{MONTHLY_SUMMARY.map((m,i)=><tr key={i} style={{opacity:m.income||m.expense?1:.4}}><td style={{...td,fontWeight:600}}>{m.month}</td><td style={{...td,...mn,fontSize:11,color:"#10b981"}}>{ff(m.income)}</td><td style={{...td,...mn,fontSize:11,color:"#f87171"}}>{ff(m.expense)}</td><td style={{...td,...mn,fontSize:11}}>{ff(m.loanGiven)}</td><td style={{...td,...mn,fontSize:11}}>{ff(m.loanTaken)}</td><td style={{...td,...mn,fontSize:11}}>{ff(m.loanRepaid)}</td><td style={{...td,...mn,fontSize:11}}>{ff(m.loanReceivedBack)}</td><td style={{...td,...mn,fontSize:11,fontWeight:700,color:m.netMovement>=0?"#10b981":"#f87171"}}>{ff(m.netMovement)}</td><td style={{...td,...mn,fontSize:11,color:m.loanImpact>=0?"#10b981":"#f87171"}}>{ff(m.loanImpact)}</td><td style={{...td,...mn,fontSize:11}}>{ff(m.accountNet)}</td><td style={{...td,...mn,fontSize:11,color:"#a5b4fc"}}>{ff(m.officeExpense)}</td><td style={{...td,...mn,fontSize:11,color:"#fbbf24"}}>{ff(m.personalExpense)}</td></tr>)}</tbody></TW></div>}
