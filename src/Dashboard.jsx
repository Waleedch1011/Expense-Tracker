  ...
  return(<div style={{minHeight:"100vh",background:"linear-gradient(180deg,#0c0f1a 0%,#111827 50%,#0f172a 100%)",color:"#e2e8f0",fontFamily:"'Inter',-apple-system,sans-serif",padding:"20px 16px"}}>
    ...
    {showAdd&&<div style={{...cd,marginBottom:20,borderColor:"rgba(99,102,241,.3)"}}><ST icon="✏️">New Transaction</ST>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:16,marginBottom:16}}>
        {[["Date","d","date"],["Type","t","select",settings.types],["Category","c","select",settings.categories],["Group","g","select",settings.groups],["Amount (PKR)","a","number"]].map(([label,key,type,opts])=>(
          <div key={key}>
            <label style={{fontSize:10,color:"#94a3b8",display:"block",marginBottom:6,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{label}</label>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:16}}>
        {[["Description","desc","text"],["From Account","fa","select",["—",...settings.accounts]],["To Account","ta","select",["—",...settings.accounts]],["Party","p","select",["—",...settings.loan_people]],["Notes","n","text"]].map(([label,key,type,opts])=>(
          <div key={key}>
            <label style={{fontSize:10,color:"#94a3b8",display:"block",marginBottom:6,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{label}</label>
            {type==="select"?<select value={newTx[key]} onChange={e=>setNewTx({...newTx,[key]:e.target.value==="—"?"":e.target.value})} style={{...ip,width:"100%",boxSizing:"border-box"}}>{opts.map(o=><option key={o} value={o==="—"?"":o}>{o}</option>)}</select>:<input type={type} placeholder="..." value={newTx[key]} onChange={e=>setNewTx({...newTx,[key]:e.target.value})} style={{...ip,width:"100%",boxSizing:"border-box"}}/>}
          </div>
        ))}
      </div>
      <div style={{marginTop:16,display:"flex",gap:10}}><button onClick={handleAdd} disabled={saving} style={{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",border:"none",color:"#fff",padding:"10px 24px",borderRadius:10,cursor:saving?"wait":"pointer",fontWeight:700,fontSize:13,opacity:saving?.6:1}}>{saving?"Saving...":"Save"}</button><button onClick={()=>setShowAdd(false)} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",color:"#94a3b8",padding:"10px 24px",borderRadius:10,cursor:"pointer",fontWeight:600,fontSize:13}}>Cancel</button></div>
    </div>}
